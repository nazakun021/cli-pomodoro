...
final class SummaryTests: XCTestCase {
    func testSummaryStorePersistsAndDeduplicatesFocusContribution() throws {
        ...
        XCTAssertEqual(store.summary(for: "2026-08-10"), DailySummary(focusMilliseconds: 90_001, completedRounds: 1))
        let reloaded = try SummaryStore(fileURL: url)
        XCTAssertEqual(reloaded.summary(for: "2026-08-10"), DailySummary(focusMilliseconds: 90_001, completedRounds: 1))
    }
    ...
    func testSummaryStoreKeepsSplitContributionsForOnePhase() throws {
        ...
        try store.record(FocusContribution(
        ...
        XCTAssertEqual(store.summary(for: "2026-08-11"), DailySummary(focusMilliseconds: 2_000, completedRounds: 1))
    }
}
