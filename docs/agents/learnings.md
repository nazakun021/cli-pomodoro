...
- Trigger: several background operations deliver AppKit work through one main-run-loop dispatcher. Rule: queue every pending action in order; a single replaceable slot can silently drop mutation completions when periodic refresh overlaps them. Evidence: the focused dispatcher test observed `[2]` instead of `[1, 2]` before the queue fix and passes afterward.
- Trigger: an `NSMenuItem` uses a short-lived custom target object. Rule: retain the target strongly on the menu item, such as through `representedObject`; AppKit's target reference does not retain it. Evidence: the focused test observed the weak target become nil until the menu item retained it explicitly.
