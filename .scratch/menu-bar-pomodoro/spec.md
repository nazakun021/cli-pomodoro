...
Pomo is a native, menu-bar-only macOS product with a persistent Agent and a companion `pomo` CLI. The Agent is the sole owner of authoritative Session state, Phase deadlines, serialized mutations, capability-gated notifications, sounds, settings, summaries, and local persistence. The CLI is a short-lived command surface or an explicitly attached observer; closing it never ends an Agent-owned Session.
...
90. As a user I want sound enabled by default and independently configurable, plus notification settings when the running build has Apple notification capability, so that completion feedback matches my environment.
...
92. As a first-Session user of a capable signed build I want notification purpose explained before permission is requested, while an ad-hoc build never requests unavailable permission.
93. As a first-Session user of a capable signed build I want timing to start regardless of permission response timing, so that the system prompt cannot block the Session.
94. As a user I want pending or unavailable notification completions skipped rather than queued, so that stale notifications never arrive later.
95. As a user I want menu feedback and enabled sound despite pending, denied, or unavailable notifications, so that Phase completion remains observable.
96. As a user I want a temporary non-color missed-alert indicator after a pending or unavailable notification completion, so that I know a notification was unavailable.
...
99. As a user with denied notifications in a capable signed build I want Alerts to offer Open System Settings; in an ad-hoc build I want Alerts to explain the signing limitation while preserving Sound controls.
100. As a Ready-state user of a capable signed build I want completion notifications to offer Start Next Phase, so that the notification action is valid and useful.
101. As an auto-transition user of a capable signed build I want completion notifications to open current status without a stale Start action, so that notification actions match Agent state.
...
- The persistent native Agent is the sole authority for Session state, Phase timing, capability-gated notifications, sound, persistence, Recovery, settings, and summaries. The CLI and native UI are adapters over that authority and never own parallel Session state.
...
- Notification permission is explained on first Session start without delaying timing only when Apple notification capability exists. Pending or unavailable completions are skipped rather than queued and create an accessible temporary missed-alert indicator while menu feedback and enabled sound continue. Signed-build denial exposes system settings. Only Ready notifications expose Start Next Phase.
- The bundled chime is globally toggleable, enabled by default, and uses ordinary macOS output volume. System notifications are conditional on Apple team signing; ad-hoc builds do not request authorization and retain sound/menu/missed-alert feedback. Launch at login is opt-in through the supported service-registration API and exposes failures without turning an optional denial into core failure.
...
- XCUITest is used only at the native macOS UI boundary. It covers status-item and menu flows, adaptive controls, Custom Session and Welcome popovers, Settings tabs, Preset management, summary layout, confirmation scope, Recovery actions, capability-appropriate Alerts routing, launch-at-login errors, keyboard focus order, VoiceOver labels, and non-color cues while isolating real user data and login registration.
...
- Manual platform validation is reserved for boundaries automation cannot faithfully simulate: real sleep and wake including deadline races, sound, login registration, VoiceOver, Increase Contrast, Reduce Motion, light and dark appearances, Gatekeeper Open Anyway, clean-profile onboarding, Homebrew install/upgrade/uninstall/zap, direct archive installation, and supported hardware and operating-system combinations. Notification allowed/denied/pending/disabled states and actions are validated when an Apple team-signed build is available.
